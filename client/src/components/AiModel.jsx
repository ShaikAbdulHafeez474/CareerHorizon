...
Provide a concise, well-structured summary (2-3 sentences) highlighting key qualifications, experience, and achievements from the following resume text.
Format it clearly with bullet points or checkmarks for key points.
...
Analyze the resume below and provide 3-5 **specific, actionable suggestions** for improvement. 
Focus on skills presentation, quantifying achievements, clarity, structure, formatting, and alignment with common job requirements. 

Format the suggestions **clearly** with:
- ✅ for strong points
- ⚠️ for areas needing improvement
- Bullet points for readability
- Use Markdown formatting for bold and clarity
...
---`;
    ...
    const getMissingKeywords = async () => {
       ...
       const prompt = `You are an expert ATS analyzer. Compare the following resume against the job description. 
List all key skills, technologies, or qualifications from the job description that are missing or underrepresented in the resume. 
Provide the output in a **structured, clear, bullet-pointed format** with:
- ✅ for present skills
- ⚠️ for missing/underrepresented skills
...
---`;       
    ...
    const getPercentageMatch = async () => {
       ...
       const prompt = `You are an expert ATS and resume reviewer. 
Analyze the alignment between the resume and job description, considering both **content and formatting**. 
Provide a **structured, point-wise, Markdown-formatted analysis**. Follow these instructions:

1. Break the analysis into the following sections:
   - Keywords & Skills
...
2. For each section:
   - Use ✅ for strong points
   - Use ⚠️ for weaknesses or areas needing improvement
   - Use bullet points for clarity
   - Bold important observations

3. At the end, provide a **"Specific Improvements"** section with clear actionable suggestions.

4. Start the first line with the **overall percentage match score (0-100)** in this format:
   Score: [percentage]
                ...
                <div className='router-container'>
                    ...
                    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8"> {/* Wider container, responsive padding */}
                        <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">
                            AI Resume Analyzer <span className="bg-gradient-to-r from-[#0B535B] to-[#412B5E] bg-clip-text text-transparent">powered by Gemini</span>
                        </h1>
                        ...
                        {resumeText && isClientInitialized && (
                            <div className="space-y-8">
                                ...
                                <div className="bg-white p-5 rounded-lg shadow">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        <button onClick={getSummary} disabled={isLoading || !resumeText} className="px-4 py-2 bg-gradient-to-r from-[#0B535B] to-[#412B5E] text-white text-sm font-medium rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            Summary
                                        </button>
                                        <button onClick={getImprovements} disabled={isLoading || !resumeText} className="px-4 py-2 bg-gradient-to-r from-[#0B535B] to-[#412B5E] text-white text-sm font-medium rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            Improvements
                                        </button>
                                        <button onClick={getMissingKeywords} disabled={isLoading || !resumeText || !jobDescription} className="px-4 py-2 bg-gradient-to-r from-[#0B535B] to-[#412B5E] text-white text-sm font-medium rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            Missing Keywords
                                        </button>
                                        <button onClick={getPercentageMatch} disabled={isLoading || !resumeText || !jobDescription} className="px-4 py-2 bg-gradient-to-r from-[#0B535B] to-[#412B5E] text-white text-sm font-medium rounded-md shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            Calculate % Match
                                        </button>
                                    </div>
                                </div>
                         ...
                         <Markdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} className="prose prose-xs prose-headings:text-sm prose-p:text-xs prose-li:text-xs max-w-none text-gray-800">
                                                    {analysisResult}
                                                </Markdown>
                                ...
                                <div className="bg-white p-5 rounded-lg shadow">
                                 ...
                                 <Markdown className="prose prose-xs prose-headings:text-sm prose-p:text-xs prose-li:text-xs max-w-none text-gray-800"
